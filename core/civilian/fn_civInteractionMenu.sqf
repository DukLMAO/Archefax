...
_Btn2 buttonSetAction "if(player distance life_pInact_curTarget > 4) exitWith {hint 'You are too far away.'}; [life_pInact_curTarget] spawn life_fnc_unrestrain; closeDialog 0;";
...
if(!((player distance (getMarkerPos "jail_marker_v1") < 55) OR (player distance (getMarkerPos "jail_marker_v2") < 55) OR (player distance (getMarkerPos "jail_marker_v3") < 55) OR (player distance (getMarkerPos "jail_marker_v4") < 55))) then
{
	_Btn6 ctrlEnable false;
};
