...
{
	if(zg_wanted_status > 0) then
	{
		LIFEctrl(IDC_LIFE_BAR_WANTED)ctrlSetBackgroundColor [1, 0, 0, .5];
		LIFEctrl(IDC_LIFE_BAR_WANTED) progressSetPosition (100);
		LIFEctrl(IDC_LIFE_WANTED_TEXT) ctrlSetText format["$%1", ([zg_wanted_status] call life_fnc_numberText)];
	}
	else
	{
...
{
	LIFEctrl(IDC_LIFE_BAR_WANTED)ctrlSetBackgroundColor [0, 1, 0, 1];
	LIFEctrl(IDC_LIFE_BAR_WANTED) progressSetPosition (100);
	LIFEctrl(IDC_LIFE_WANTED_TEXT) ctrlSetText "On Duty";
};
