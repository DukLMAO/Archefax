...
*/
private["_curTarget","_isWater","_isCrate","_lists"];
_curTarget = cursorTarget;
if(life_action_inUse) exitWith {}; //Action is in use, exit to prevent spamming.
if(life_action_gathering) exitWith {};
if(life_interrupted) exitWith {life_interrupted = false;};
_isWater = surfaceIsWater (visiblePositionASL player);
_lists = ["Land_CargoBox_V1_F"];
_isCrate = if(KINDOF_ARRAY(_curTarget,_lists)) then {true} else {false};
if(_isCrate) then {
	if(!dialog) then {
		if(player distance _curTarget < 3) then {
			[_curTarget] call life_fnc_cInteractionMenu;
		};
	};
};
