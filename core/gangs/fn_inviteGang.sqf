...
if (_send) then
{
	...
	closeDialog 0;
	[[player,life_gang, player getVariable ["gang_name"]],"life_fnc_invitedGang",_info,false] spawn BIS_fnc_MP;
}
else
{
	...
	_playableUnits = [];
	{
		if ((side _x) == civilian && (_x getVariable ["gang_name", ""]) == "") then { _playableUnits set [count _playableUnits, name _x]; };
