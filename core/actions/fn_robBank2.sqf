...
private["_robber","_shop","_kassa","_ui","_progress","_pgText","_cP","_rip","_pos","_lastrobbed","_cpRate"];
_shop = [_this,0,ObjNull,[ObjNull]] call BIS_fnc_param; //The object that has the action attached to it is _this. ,0, is the index of object, ObjNull is the default should there be nothing in the parameter or it's broken
_robber = [_this,1,ObjNull,[ObjNull]] call BIS_fnc_param; //Can you guess? Alright, it's the player, or the "caller". The object is 0, the person activating the object is 1
_action = [_this,2] call BIS_fnc_param;//Action name
_pos = GetPos player;

if({side _x == west} count playableUnits < 2) exitWith {hint "There must be two or more cops online for you to initiate a robbery!";};
...
if(life_crob) exitWith {hint "The bank was recently robbed and contains no money"};
...
_rip = true;
_kassa = 30000 + round(random 30000); //setting the money in the registry, anywhere from 4000 to 12000.
...
_Pos = position player;
            _marker = createMarker ["Marker200gggsdgesdgg", _Pos];
            "Marker200gggsdgesdgg" setMarkerColor "ColorRed";
            "Marker200gggsdgesdgg" setMarkerText "!DANGER! Robbery in progess !DANGER!";
            "Marker200gggsdgesdgg" setMarkerType "mil_warning";

/*[[3, format["<t size='3'><t color='#FF0000'>ROBBERY</t></t> <br/><t size = '1.5'>%1 is robbing a local Bank!!", name player]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;*/

[[1, format["%1 is Robbing the Bank of Altis! Go and stop them fast!", name player]],"life_fnc_broadcast",west,false] spawn life_fnc_MP; //Give the cops a hint
[[4, format["%1 is Robbing the Bank of Altis!", name player]],"life_fnc_broadcast",true,false] spawn life_fnc_MP; //Give everyone a systemchat
...
if(_rip) then
{
   ...
   deleteMarker "Marker200gggsdgesdgg";
