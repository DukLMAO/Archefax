...
if (_unit getVariable["Escorting", false]) exitWith {
    hint "You must set the player down first.";
};
if (!isNull(_unit getVariable["TransportingPlayer", objNull])) exitWith {
    hint "You must set the player down first.";
};
...
if (_vigi && ((_amount < 30000) || !finite _amount)) exitWith {hint "You can only arrest players with a bounty larger than $30,000! Please let this player go."; [_unit] call life_fnc_unrestrain;};
