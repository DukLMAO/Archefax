...
/*
	...
	Edited for Territories by Ethan
...
//if({side _x == civilian} count playableUnits < 10) exitWith {hint "There must be at least 10 civilians online in order to take over the Arms Dealer";};
if(isNil {grpPlayer getVariable "gang_name"}) exitWith {titleText[localize "STR_GNOTF_CreateGang","PLAIN"];};
if(_group == grpPlayer) exitWith {titleText[localize "STR_GNOTF_Controlledt","PLAIN"]};
if((_hideout getVariable ["inCapture",FALSE])) exitWith {hint localize "STR_GNOTF_Captured";};
if(!isNull _group) then {
	_gangName = _group getVariable ["gang_name",""];
	_action = [
		format[localize "STR_GNOTF_AlreadyControlledt",_gangName],
		localize "STR_GNOTF_CurrentCapturet",
...
[[[0,1],"STR_GNOTF_CaptureAttemptad",true,[name player,(group player) getVariable "gang_name"]],"life_fnc_broadcast",true,false] call life_fnc_MP;
...
_title = localize "STR_GNOTF_Capturingt";
titleText["!!!STAY WITHIN 100 METERS TO CAPTURE!!!","PLAIN"];
...
while {true} do
{
	...
	if(player distance _hideout > 100) exitWith {_hideout setVariable["inCapture",false,true];};
};
...
if(player distance _hideout > 100) exitWith {life_interrupted = false; titleText[localize "STR_GNOTF_CaptureCancel","PLAIN"]; life_action_inUse = false;_hideout setVariable["inCapture",false,true];};
//life_action_inUse = false;

titleText["Territory has been captured.","PLAIN"];
_flagTexture = [
...
[[[0,1],"STR_GNOTF_CaptureSuccesstad",true,[name player,(group player) getVariable "gang_name"]],"life_fnc_broadcast",true,false] call life_fnc_MP;
