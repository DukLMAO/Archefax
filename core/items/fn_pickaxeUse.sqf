...
switch (true) do {
	case (player distance (getMarkerPos "lead_1") < 30): {_mine = ["copper_unrefined",4];};
	case (player distance (getMarkerPos "iron_1") < 30): {_mine = ["iron_unrefined",4];};
	...
	case (player distance (getMarkerPos "diamond_1") < 50): {_mine = ["diamond_uncut",4];};
	case (player distance (getMarkerPos "oil_1") < 40) : {_mine = ["oil_unprocessed",4];};
	case (player distance (getMarkerPos "oil_2") < 40) : {_mine = ["oil_unprocessed",4];};
	case (player distance (getMarkerPos "rock_1") < 50): {_mine = ["rock",4];};
	case (player distance (getMarkerPos "gold_1") < 50): {_mine = ["goldu",4];};
	default {["", 0]};
};
//Mine check
