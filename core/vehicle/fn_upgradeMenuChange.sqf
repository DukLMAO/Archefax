...
_baseprice = M_CONFIG(getNumber,CONFIG_VEHICLES,typeOf _vehicle,"insurance");
...
switch (_ins) do {
	...
	case 3:
	{
		_insprice = 0.80 * _baseprice + 0.3 * (_hooksprice + _trunkprice + _gpsprice + _securityprice);
	};
	default
	{
		_insprice = 0;
...
switch (_oldins) do {
	...
	case 3:
	{
		_oldinsprice = 0.80 * _baseprice + 0.3 * (_oldhooksprice + _oldtrunkprice + _oldgpsprice + _oldsecurityprice);
	};
	default
	{
		_oldinsprice = 0;
