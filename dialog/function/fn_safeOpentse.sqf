...
if(life_emptyse) exitWith {hint "There are currently no taxes to collect"};
if((life_tObj getvariable ["empty",false])) exitWith {hint "There are currently no taxes to collect"};
if(EQUAL(life_emptyse,false)) then
{
	...
	sleep (30*60);
	life_emptyse = false;
	hint "South-East Territory Taxes Are Now Available";
};
