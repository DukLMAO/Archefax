...
if ( life_cash < 5000) exitWith { systemChat "You need $5000 to send a message!"; }; // Hint if person haves no 6000 dollar
if ( playerSide != civilian ) exitWith { systemChat "You need to be a civilian!"; }; // Hint if not civilian
if !( life_channel_send ) exitWith { systemChat "You have to wait 20 minutes before sending another message!"; }; //Gives Player the Hint to Wait 10 mins
