...
if(life_emptysw) exitWith {hint "There are currently no taxes to collect"};
if((life_tObj getvariable ["empty",false])) exitWith {hint "There are currently no taxes to collect"};
if(EQUAL(life_emptysw,false)) then
{
	...
	sleep (30*60);
	life_emptysw = false;
	hint "South-West Territory Taxes Are Now Available";
};
